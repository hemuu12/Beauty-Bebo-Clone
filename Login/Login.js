...
let form = document.querySelector("form").addEventListener("submit", loginFun);
...
function loginFun(event) {
  ...
  if (checkLogin(objLogin.l_email, objLogin.l_password) === true) {
    localStorage.setItem("loginData", JSON.stringify(objLogin));
...
document.querySelector("#fblogin").addEventListener("click", fbFun);

function fbFun(){
  window.location.href="https://www.facebook.com/login/"
}

document.querySelector("#googlelogin").addEventListener("click", googleFun);

function googleFun(){
  window.location.href="https://accounts.google.com/signin/v2/identifier?service=accountsettings&continue=https%3A%2F%2Fmyaccount.google.com%2F&ec=GAlAwAE&flowName=GlifWebSignIn&flowEntry=AddSession"
}
